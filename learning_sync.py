...
moodle_data = conn.execute('SELECT id, email, idnumber, institution, department, city, country FROM mdl_user WHERE auth="'+aa+'";')
...
for email, (dbid, dbempl, dbinstit, dbdepart, dbcity, dbcountry) in moodle_map.items():
    if email in users:
        ...
        if 'org_data' in users[email] and 'units' in users[email]['org_data']:
            department = next( (x['name'] for x in reversed( users[email]['org_data']['units'] ) if x['unit_type'].startswith('Department')), None )
            institution = next( (x['name'] for x in reversed( users[email]['org_data']['units'] ) if x['unit_type'].startswith('Division')), None )
            city = next( (x['location__name'] for x in reversed( users[email]['org_data']['units'] ) if x['unit_type'].startswith('Division')), None )
            ...
            if city is not None and city != dbcity:
               print('Updating Moodle: set {} city from {} to {}'.format(email, dbcity, city))
            ...
            if department is not None and department != dbdepart:
               print('Updating Moodle: set {} department from {} to {}'.format(email, dbdepart, department))
               statement = text('UPDATE mdl_user SET department = :department WHERE id = :row_id')
               conn.execute(statement, department=department, row_id=dbid)
            if dbcountry is None or  dbcountry != 'AU':
               country = 'AU'
               print('Updating Moodle: set {} country from {} to {}'.format(email, dbcountry, country))
               statement = text('UPDATE mdl_user SET country = :country WHERE id = :row_id')
